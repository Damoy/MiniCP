...
public class TSP {
        ...
        DFSearch dfs = makeDfs(cp,
                selectMin(succ,
                        ...
                        succi -> {
                        	...
                        	// change this to get the nearest city and thus
                        	// help the search 
                        	
                        	int i = getDistIndex(succ, distSucc, succi);
                        	int v = getMinIndex(distanceMatrix[i], succi);
    ...
    private static int getMinIndex(int[] array, IntVar succi) {
    	int min = Integer.MAX_VALUE;
    	int imin = -1;
    	
    	for(int i = succi.getMin(); i < succi.getMax(); i++) {
    		if(array[i] < min && succi.contains(i)) {
    			min = array[i];
    			imin = i;
    		}
    	}
    ...
    private static int getDistIndex(IntVar[] succ, IntVar[] distSucc, IntVar succi) {
    	assert(succ.length == distSucc.length);
    	for(int i = 0; i < succ.length; i++) {
    		if(succ[i] == succi)
    			return i;
    	}
    	throw new IllegalStateException("succi does not exist in succ");
    }
    
//    private static IntVar getDist(IntVar[] succ, IntVar[] distSucc, IntVar succi) {
//    	assert(succ.length == distSucc.length);
//    	for(int i = 0; i < succ.length; i++) {
//    		if(succ[i] == succi) {
//    			return distSucc[i];
...
//    
//    // succ wrong
//    private static int getMin(IntVar[] succ, IntVar distSucci) {
//    	int min = Integer.MAX_VALUE;
//    	for(int i = distSucci.getMin(); i < distSucci.getMax(); i++) {
//    		if(contains(succ, i) && i < min) {
//    			min = i;
//    		}
//    	}
//    	
...
//    private static boolean contains(IntVar[] succ, int value) {
//    	for(int i = 0; i < succ.length; i++) {
//    		IntVar succi = succ[i];
//    		
//    		if(ok(succi)) {
//    			if(succi.getMin() == value)
//    				return true;
//    		}
//    	}
//    	
...
//    }
    ...
    private static boolean ok(IntVar succi) {
    	return succi != null && succi.isBound();
    }
}